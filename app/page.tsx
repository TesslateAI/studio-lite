...
const FeatureCard = ({ icon, title, children }: { icon: React.ReactNode, title: string, children: React.ReactNode }) => (
    <div className="bg-card/50 dark:bg-card/30 p-6 rounded-2xl border border-border/50 backdrop-blur-sm transition-all duration-300 hover:shadow-xl hover:border-[#5E62FF]/30">
        <div className="flex items-center justify-center h-12 w-12 rounded-full bg-[#5E62FF]/10 text-[#5E62FF] mb-4">
            {icon}
        </div>
    ...
    return (
        <section className="relative overflow-hidden pt-28 md:pt-36 pb-20 md:pb-24">
            <div aria-hidden="true" className="absolute inset-x-0 top-0 -z-10 transform-gpu overflow-hidden blur-3xl">
                <div
                    className="relative left-[calc(50%-11rem)] aspect-[1155/678] w-[36.125rem] -translate-x-1/2 rotate-[30deg] bg-gradient-to-tr from-[#5E62FF]/40 to-primary/30 opacity-30 sm:left-[calc(50%-30rem)] sm:w-[72.1875rem]"
            ...
            <div className="container mx-auto px-6 text-center">
                ...
                <div className="flex justify-center items-center gap-4">
                    <Button size="lg" className="bg-[#5E62FF] hover:bg-[#7A7DFF] text-white shadow-lg hover:shadow-xl transition-all duration-200" onClick={() => router.push('/chat')}>
    ...
    <section id="features" className="py-20 md:py-28 bg-secondary/30 dark:bg-muted/10">
        <div className="container mx-auto px-6">
            <div className="max-w-3xl mx-auto text-center mb-16">
                <h2 className="text-3xl sm:text-4xl font-medium tracking-tight mb-4">A New Way to Build for the Web</h2>
...
const UseCasesSection = () => (
    <section id="use-cases" className="py-20 md:py-28">
        <div className="container mx-auto px-6">
             <div className="max-w-3xl mx-auto text-center mb-16">
                <h2 className="text-3xl sm:text-4xl font-medium tracking-tight mb-4">What will you build?</h2>
                <p className="text-lg text-muted-foreground">
    ...
    <section id="pricing" className="py-20 md:py-28 bg-secondary/30 dark:bg-muted/10">
        <div className="container mx-auto px-6">
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-3 lg:gap-6 items-stretch">
                {plans.map((plan) => (
                    <div key={plan.name} className={`flex flex-col rounded-xl border bg-card p-6 shadow-lg transition-all duration-300 ${plan.highlight ? "border-[#5E62FF] ring-2 ring-[#5E62FF]/70 scale-[1.02] lg:scale-105" : "border-border hover:shadow-xl"}`}>
                        {plan.highlight && (
                            <div className="mb-4 text-center">
                                <span className="inline-block rounded-full bg-[#5E62FF] px-3 py-1 text-xs font-semibold uppercase tracking-wider text-white">Most Popular</span>
                            </div>
                        )}
                        ...
                        <ul className="space-y-2 text-sm text-muted-foreground flex-grow">
                            {plan.features.map((feature) => (
                                <li key={feature} className="flex items-center"><CheckCircle2 className="mr-2 h-4 w-4 flex-shrink-0 text-[#5E62FF]" />{feature}</li>
                            ))}
                        </ul>
                        <Button onClick={() => handlePlanSelection(plan)} size="lg" className={`mt-8 w-full ${plan.highlight ? "bg-[#5E62FF] hover:bg-[#7A7DFF] text-white" : "bg-primary text-primary-foreground hover:bg-primary/90"}`}>{plan.cta}</Button>
                    </div>
                ))}
            </div>
        </div>
...
const FinalCTASection = () => {
    ...
    return (
        <section className="py-20 md:py-28">
            <div className="container mx-auto px-6 text-center">
                <h2 className="text-3xl sm:text-4xl font-medium tracking-tight mb-4">Ready to build faster?</h2>
                ...
                <Button size="lg" className="bg-[#5E62FF] hover:bg-[#7A7DFF] text-white shadow-lg hover:shadow-xl transition-all duration-200" onClick={() => router.push('/sign-up')}>
